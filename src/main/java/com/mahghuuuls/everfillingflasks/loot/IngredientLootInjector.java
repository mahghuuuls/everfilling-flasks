...
/**
 * Puts the built-in ingredients into common exploration chests — the owner's 2026-08-25
 ...
 * pool is behind {@code general.ingredientLoot}; off means no pool is ever added.
 */
@Mod.EventBusSubscriber(modid = Tags.MOD_ID)
public final class IngredientLootInjector {
    ...
    private IngredientLootInjector() {
    }

    @SubscribeEvent
    public static void onLootTableLoad(LootTableLoadEvent event) {
        if (!ConfigSnapshot.current().ingredientLoot() || !TARGETS.contains(event.getName())) {
            return;
        }
        LootEntry[] entries = new LootEntry[]{
                herb(IngredientKind.SUNPETAL_LEAF, 30),
                herb(IngredientKind.IRONROOT_SPRIG, 30),
                herb(IngredientKind.QUICKMINT_LEAF, 30),
                herb(IngredientKind.SECOND_WIND_PETAL, 15),
        ...
        event.getTable().addPool(new LootPool(entries, new LootCondition[0],
                ...
                Tags.MOD_ID + "_ingredients"));
    }

    private static LootEntryItem herb(IngredientKind kind, int weight) {
        return new LootEntryItem(ModItems.ingredient(kind), weight, 0,
